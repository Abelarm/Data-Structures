...
public class NodePositionList<E> implements PositionList<E> {
	...
	public DNode<E> checkPosition(Position<E> p){		
		...
		if(p==header) throw new InvalidPositionException("Header non ï¿½ una position valida");
		if(p==trailer) throw new InvalidPositionException("Trailer non ï¿½ una position valida");
	...
	public static <E> void merge(NodePositionList<E> in1, NodePositionList<E> in2, Comparator <E> c, NodePositionList<E> in){
	...
	public static <E> void mergeSort(PositionList<E> in, Comparator<E> c){
		...
		while(i<(n/2)){																				//finche questo contatore è minore della dimensione della (lista/2)/2
			in1.addLast(in.remove(in.first()));															//togliamo dalla lista il primo elemento e lo inseriamo in una delle due liste d'appoggio create															
			i++;;																						//incrementiamo il contatore
		}
		while(i<n){																					//finche il contatore di prima(che sarà arrivato (n/2)2	 è minore della dimensione																	
			in2.addLast(in.remove(in1.first()));														//togliamo dalla lista il primo elemento e lo inseriamo in una delle due liste d'appoggio create	
			i++;																						//incrementiamo il contatore
		}
