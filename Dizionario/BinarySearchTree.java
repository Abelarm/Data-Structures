...
/**Implementazione del dizionario ordinato attraverso un albero binario aventi le seguenti proprieta:
 * left(v)<=v<=right(v) cioè il figlio sinistro è piu piccolo del padre che ha sua volta è piu piccolo del figlio destro**/
public class BinarySearchTree<K, V> extends LinkedBinaryTree<Entry<K,V>> implements OrderedDictionary<K, V> {
	...
	public BinarySearchTree() {
		...
		addRoot(null); //perchè non aumento size???
	} 
   ...
   /**Controlla se la entry è valida**/
   ...
   protected Position<Entry<K,V>> treeSearch(K key, Position<Entry<K,V>> pos) { 
	   if (isExternal(pos)) 												//se è sentinella key non estite nel sottoalbero radicato in pos
		   return pos; 																		//ritorniamo pos stesso che è uguale a null
	   else { 
   ...
   /**Restituisce true se è vuota altrimenti false**/
   ...
   return L;														//restituisce la lista creata
	...
	public Iterable<Entry<K, V>> entries() {
		...
		this.inorderPositions(root, temp);																		//richiama il metodo inorderPosition partendo dalla root
	...
	public Entry<K,V> remove(Entry<K,V> ent) throws InvalidEntryException { 
		...
		if (isExternal(left(remPos))) 										//controlla se il figlio sinistro é esterno
			...
			remPos = right(swapPos); 										//nella posizione da eliminare inserisce il figlio destro della swap position(cioò verso quelli maggiori di lui)
			do 
	...
	/**Restituisce il primo elemento dell'albero(quello con la key piè piccola)**/
	...
	/**Restituisce il primo elemento dell'albero(quello con la key piè grande)**/
	...
	public Iterable<Entry<K, V>> successors(K key) {
		...
		PositionList<Position<Entry<K,V>>> temp=new NodePositionList<Position<Entry<K,V>>>();		//Creaiamo una PositionList di Position di entry d'appoggio
		this.inorderPositions(this.root(), temp);													//invochaimo il meotodo in order su actionPos(cioé il figlio destro, in questo modo avremo una lista contentente le Position di entry con chiave piè grande di quella passsata in input cioè i successori)
		NodePositionList<Entry<K,V>> lista= new NodePositionList<Entry<K,V>>();						//Creiamo una PositionList di Entry
