...
/**Implementazione del dizionario attraverso un array di dizionari dove in ogni cella c'è il puntatore ad un dizionario
 *gestione delle collisioni con il separate chaining
 */
public class ChainingHashTable<K, V> implements Dictionary<K, V> {
	...
	public ChainingHashTable(){ //aggiungere altri costruttoriè
		capacity = 500;
	...
	public Iterable<Entry<K, V>> entries() {
		PositionList<Entry<K,V>> lista= new NodePositionList<Entry<K,V>>();					//Crea una lista d'appoggio(che sarè la nostra collezione iterabile)
		...
		for (int i=0; i<capacity; i++){														//Per ogni lista contenuta in A[i] 
			...
			while(temp.hasNext()){																	//finche l'iteratore ha un successivo
				...
				if(!x.equals(null))																		//se la entry è diversa da null
	...
	public Iterable<Entry<K, V>> findAll(K key) throws InvalidKeyException {
		this.checkKey(key);							//controlla la validitè della key
		int x= this.hashValue(key);					//dentro x assegna il valore hash della key passata in input
		return A[x].entries();						//richiama su A[x](cioè la cella in cui ci sono tutte le chiavi aventi funzione hash=alla funzione hash di key) il metodo entrys(cioè restituisce una iterable di Entry)
	}
	...
	public Entry<K, V> insert(K key, V value) throws InvalidKeyException {
		...
		if(size==capacity-1)
			this.rehash();
		int x= this.hashValue(key);					//dentro x assegna il valore hash della key passata in input
		Entry<K,V> e= A[x].insert(key, value);		//richiama su A[x](cioè la cella in cui ci sono tutte le chiavi aventi funzione hash=alla funzione hash di key) l'insert con gli stessi parametri passati in input
	...
	/**Restituisce true se la il dizionario è vuoto altrimenti false**/
	...
	public Entry<K, V> remove(Entry<K, V> e) throws InvalidEntryException {
		...
		Entry<K,V> temp=A[x].remove(e);						//richiama su A[x](cioè la cella in cui ci sono tutte le chiavi aventi funzione hash=alla funzione hash di key) remove e come parametro il parametro passato in input
		return temp;
	...
	public void UpdateKey(K key, K newKey){
		this.checkKey(key);																		//controlla la validitè della key
		this.checkKey(newKey);																	//controlla la validitè della key
